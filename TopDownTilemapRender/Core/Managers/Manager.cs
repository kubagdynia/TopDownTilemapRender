...
namespace TopDownTilemapRender.Core.Managers
{
    public class Manager<T> : IManager<T> where T : class
    {
        ...
        public void LoadOnly(string name, string filename) => LoadOnly(name, filename, false);

        public void LoadOnly(string name, string filename, bool overrideItem) => LoadOnly(name, filename, overrideItem, null);

        public void LoadOnly(string name, string filename, object parent) => LoadOnly(name, filename, false, parent);

        public void LoadOnly(string name, string filename, bool overrideItem, object parent)
        {
            if (string.IsNullOrWhiteSpace(name) || string.IsNullOrWhiteSpace(filename))
            {
                return;
